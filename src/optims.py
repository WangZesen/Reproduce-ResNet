...
from src.conf import (
    ...
    AdamConfig,
    SGDConfig,
    SGDScheduleFreeConfig,
    CosineLRSchedulerConfig,
    AdamWScheduleFreeConfig,
    SGDSAMConfig,
)
from schedulefree import SGDScheduleFree, AdamWScheduleFree
from src.custom_optims.sam import SAMV1, S2SAM
...
def get_optim_fn(cfg: Config, num_steps_per_epoch: int) -> Callable[[List[Tuple[torch.Tensor, str]]], Optimizer]:
    ...
    match optim_cfg.name.lower():
        case "adam":
            ...
            def adam_fn(
                params: List[Tuple[torch.Tensor, str]],
                lr: float,
                betas: Tuple[float, float],
                eps: float = optim_cfg.epsilon,
            ) -> Optimizer:
                return Adam(get_param_groups_from_list(params, optim_cfg.weight_decay), lr=lr, betas=betas, eps=eps)
        ...
        case "sgd":
            ...
            def sgd_fn(params: List[Tuple[torch.Tensor, str]], lr: float, momentum: float) -> Optimizer:
                return SGD(get_param_groups_from_list(params, optim_cfg.weight_decay), lr=lr, momentum=momentum)
        ...
        case "sgd-schedule-free":
            ...
            def sgd_schedule_free_fn(
                ...
                return SGDScheduleFree(
                    get_param_groups_from_list(params, weight_decay),
                    lr=lr,
                    momentum=momentum,
        ...
        case "sgd-sam":
            ...
            def sgd_sam_fn(
                params: List[Tuple[torch.Tensor, str]], lr: float, momentum: float, rho: float, adaptive: bool, v2: bool
            ) -> Optimizer:
                if not v2:
                    return SAMV1(
                        get_param_groups_from_list(params, optim_cfg.weight_decay),
                        base_optimizer=SGD,
                        lr=lr,
                        momentum=momentum,
                        rho=rho,
...
def get_lr_scheduler_fn(cfg: Config, num_steps_per_epoch: int) -> Callable[[Optimizer], LRScheduler]:
    ...
    match lr_scheduler_cfg.name.lower():
        case "cosine":
            ...
            def cosine_fn(
                optim: Optimizer, warmup_epochs: int, warmup_decay: float, max_epochs: int, eta_min: float
            ) -> LRScheduler:
                warmup_lr_scheduler = torch.optim.lr_scheduler.LinearLR(
                    optim, start_factor=warmup_decay, total_iters=num_steps_per_epoch * warmup_epochs
                )
                main_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                    optim, T_max=num_steps_per_epoch * (max_epochs - warmup_epochs), eta_min=eta_min
                )
