...
from src.data.dataloader import get_dali_train_loader, get_dali_valid_loader
...
gpu_models = (
    subprocess.check_output("nvidia-smi --query-gpu=name --format=csv,noheader", shell=True).decode("utf-8").split("\n")
)
        ...
        if rank == 0:
            ...
            if step % cfg.train.log.log_freq == 0:
                throughput_metric.update(
                    (images.size(0) * cfg.train.network.world_size) / (time.time() - iter_start_time), images.size(0)
                )
                logger.info(
                    f"step: {step} ({throughput_metric.avg:5.2f} imgs/s), loss: {loss_metric.avg:.6f}"
...
def collect_bn_stats(cfg: Config, model: Any, stats_ds: DALIGenericIterator) -> None:
    model.train()
    cnt = 0
...
def valid(
    ...
    model: Any,
    optimizer: Optimizer,
    stats_ds: DALIGenericIterator,
    valid_ds: DALIGenericIterator,
    criterion: Module,
    epoch: int,
    ...
    for data in valid_ds:
        ...
        acc1, acc5 = get_accuracy(logit, labels.view(-1), topk=(1, 5))
...
def main():
    ...
    if rank == 0:
        train_log = pd.DataFrame(
            columns=["epoch", "step", "train_loss", "val_loss", "val_acc1", "val_acc5", "time", "checkpoint_dir"]
        )
    ...
    with profile(
        ...
        on_trace_ready=torch.profiler.tensorboard_trace_handler(
            os.path.join(cfg.train.log.log_dir, "tb_trace"), worker_name=f"worker_{rank:02d}"
        ),
    ) as profiler:
        for epoch in range(start_epoch, cfg.train.max_epochs):
            if isinstance(optimizer, SAM) or isinstance(optimizer, SAMV1) or isinstance(optimizer, S2SAM):
                train_loss, global_step, epoch_train_time = train_epoch_for_sam(
                    cfg, model, train_ds, criterion, optimizer, lr_scheduler, epoch, global_step, scaler, profiler
                )
            else:
                train_loss, global_step, epoch_train_time = train_epoch(
                    cfg, model, train_ds, criterion, optimizer, lr_scheduler, epoch, global_step, scaler, profiler
                )
            total_train_time += epoch_train_time
            val_loss, val_acc1, val_acc5, val_samples = valid(
                cfg, model, optimizer, train_ds, valid_ds, criterion, epoch
            )
            ...
            if rank == 0:
                logger.info(
                    f"Epoch: {epoch + 1}, Train Loss: {stats[0]:.6f}, Val Loss: {stats[1]:.6f}"
                ...
                train_log.loc[epoch - start_epoch] = [  # type: ignore
                    epoch + 1,
                    global_step,
                    stats[0],
                    stats[1],
